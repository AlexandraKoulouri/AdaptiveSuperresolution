...
function  [cluster, ind_nodes_clusters, ind_inner_nodes_clusters,El]= clustering(El,x,y,ind_nz_el)
        ...
        while (~isempty(St))
             i = length(cur_elem_ind);
              while i>=1 &&  (~isempty(St))
                      ...
                      Dist = ( c_x(cur_elem_ind(i)) - c_x(St)).^2+(c_y(cur_elem_ind(i))-c_y(St)).^2;
                      
                      [~,j]=sort(Dist);
                      
                      
%                       if length(Dist12)>5
%                         j = find(Dist12 <=4*min(Dist12));
%                       else
%                         j = 1:length(St);
%                       end
