...
[El,ed,ind_ed_clusters]=smoothBoundary_v2(x,y,El);
...
% end
%  plot(x(ind_nz_nodes_on_boundary),y(ind_nz_nodes_on_boundary),'rx')
...
if ~isempty(ind_nz_nodes_on_boundary) 
      ...
      if length(indedg2)==2 && length(indedg1)==2 %remove points which connect two area..
             ...
             %  ed1 =boundedges([x y],El); 
%                 figure
%                 hold on
%                 for i =1:length(ed1)
%                      %plot(x(ed1(i,:)),y(ed1(i,:)),'y-') 
%                     
%                 end
%                 for i =1:length(ed)
%                      %plot(x(ed(i,:)),y(ed(i,:)),'r-') 
%                     
%                 end
...
[cluster_elem,~,~,H]= clustering(H,gx,gy,1:size(H,1));
...
for i = 1:max_cluster
    ...
    a  = max(ind_polygon_line(:,2));
    ind_polygon_line(:,2) = max_no_cluster+ind_polygon_line(:,2);
    
    ind_polyg_clusters = [ind_polyg_clusters;ind_polygon_line];
    max_no_cluster  = max_no_cluster +a;
end
