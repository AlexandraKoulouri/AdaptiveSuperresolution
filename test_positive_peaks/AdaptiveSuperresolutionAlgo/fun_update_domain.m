...
%                                 yq = y_div(ind_poly(:,1));
                             ...
                             %  [El_ind_rem] =fun_elem_outside_polygon([xq(:) yq(:)] ,mid_nodes,El_ind,Tri);
                             %  Tri(El_ind_rem,:)=[];
                             ...
                             %  Tri = alphaTriangulation(alphaShape(nodes(:,1),nodes(:,2),1);%,'HoleThreshold ',AvgDist);
                              ...
                              Hnew{incr_cluster,1} = [];
                              Hnew{incr_cluster,2} = 0; %no  mesh has been created with this node structure!
                            end  
