               ...
               for clusterno = 1: numofclusters %add new nodes to each of the clusters and create new independant-disjoint meshes
                     ...
                     if ~isempty(El_div) 
                       ...
                       if true_add == 1
                         ...
                         % nodes = unique([[extra_nodes(:,1)] [extra_nodes(:,2)]],'rows','stable');
   ...
   %     extra_temp(:,1) = [x1(Hsub(:,1))+0.5*(x1(Hsub(:,2))- x1(Hsub(:,1)));x1(Hsub(:,1))+0.5*(x1(Hsub(:,3))- x1(Hsub(:,1)));x1(Hsub(:,3))+0.5*(x1(Hsub(:,2))- x1(Hsub(:,3)))];
    %    extra_temp(:,2) = [y1(Hsub(:,1))+0.5*(y1(Hsub(:,2))- y1(Hsub(:,1)));y1(Hsub(:,1))+0.5*(y1(Hsub(:,3))- y1(Hsub(:,1)));y1(Hsub(:,3))+0.5*(y1(Hsub(:,2))- y1(Hsub(:,3)))];
        ...
        if size(Hsub,1) == 1
           ...
           %   extra_nodes_temp(:,1) = [x(Hsub(:,1))+0.5*(x(Hsub(:,2))- x(Hsub(:,1)));x(Hsub(:,1))+0.5*(x(Hsub(:,3))- x(Hsub(:,1)));x(Hsub(:,3))+0.5*(x(Hsub(:,2))- x(Hsub(:,3)))];
           %    extra_nodes_temp(:,2) = [y(Hsub(:,1))+0.5*(y(Hsub(:,2))- y(Hsub(:,1)));y(Hsub(:,1))+0.5*(y(Hsub(:,3))- y(Hsub(:,1)));y(Hsub(:,3))+0.5*(y(Hsub(:,2))- y(Hsub(:,3)))];
           %     extra_nodes_temp=[ [extra_nodes_temp(:,1);mean(x(Hsub),2)] [extra_nodes_temp(:,2);mean(y(Hsub),2)]];
