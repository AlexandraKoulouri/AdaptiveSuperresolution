...
MAX_NT_ITER     = 500;      % maximum IPM (Newton) iteration
...
% VARIABLE ARGUMENT HANDLING
t0         = min(max(1,1/max(lambda)),2*n/1e-3);
defaults   = {1e-3,true,1e-3,5000,zeros(n,1),ones(n,1),t0};
...
%         %disp(sprintf('total pcg iters = %d\n',sum(pitrs)));
    ...
    % calculate gradient
    gradphi = [At*(z*2)-(q1-q2)/t; lambda-(q1+q2)/t];
    ...
    %------------------------------------------------------------
    phi = z'*z+sum(lambda.*u)-sum(log(-f))/t;
    s = 1.0;
    gdx = gradphi'*dxu;
    for lsiter = 1:MAX_LS_ITER
        ...
        if (max(newf) < 0)
            newz   =  A*newx-y;
            newphi =  newz'*newz+sum(lambda.*newu)-sum(log(-newf))/t;
